...
params [["_civ", ObjNull, [ObjNull]], ["_invs", [], [[]]], ["_robber", false, [false]]];
...
{
	{
		...
		if(_index != -1) then
		{
			_illegal = _illegal + ((_x select 1) * ([(_x select 0)] call life_fnc_marketGetSellPrice));
		};
	} foreach _invs;
	if(_illegal > 6000) then
	{
		[getPlayerUID _civ,_civ getVariable["realname",name _civ],"482"] remoteExec ["life_fnc_wantedAdd", 2, false];
	};
	[getPlayerUID _civ,_civ getVariable["realname",name _civ],"481"] remoteExec ["life_fnc_wantedAdd", 2, false];
	[0,"STR_Cop_Contraband",true,[(_civ getVariable["realname",name _civ]),[_illegal] call life_fnc_numberText]] remoteExec ["life_fnc_broadcast", west, false];
}
	else
{
...
if(_robber) then
{
	[0,"STR_Cop_Robber",true,[(_civ getVariable["realname",name _civ])]] remoteExec ["life_fnc_broadcast", 0, false];
};