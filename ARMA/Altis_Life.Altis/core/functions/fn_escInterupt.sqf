...
_canUseControls = {
	...
	if((player getVariable["restrained",FALSE]) OR (player getVariable["Escorting",FALSE]) OR (player getVariable["transporting",FALSE]) OR (player getVariable ["arrested", false]) OR (life_istazed)) then {false} else {true};
};
...
while {true} do
{
	...
	_abortButton = (findDisplay 49) displayCtrl 104;
	_abortButton buttonSetAction "[player] remoteExec [""TON_fnc_cleanupRequest"", 2, false]";
