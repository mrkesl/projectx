...
if(!(_building isKindOf "House_F")) exitWith {hint "Du musst auf eine Haustüre gucken.";};
...
if((nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"]) == _building OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _building) then {
	[[1,2],"STR_ISTR_Bolt_AlertFed",true,[]] remoteExec ["life_fnc_broadcast", 0, false];
} else {
	[0,"STR_ISTR_Bolt_AlertHouse",true,[profileName]] remoteExec ["life_fnc_broadcast", 0, false];
};
