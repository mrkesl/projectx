...
switch (true) do
{
	case (_vehicle isKindOF "LandVehicle"):
	{
		...
		{
			[_vehicle,(Fuel _vehicle) + 0.45] remoteExec ["life_fnc_setFuel", _vehicle, false];
		}
			else
		{
			_vehicle setFuel ((Fuel _vehicle) + 0.45);
		};
	};
	...
	case (_vehicle isKindOf "Air"):
	{
		if(!local _vehicle) then
		{
			[_vehicle,(Fuel _vehicle) + 0.2] remoteExec ["life_fnc_setFuel", _vehicle, false];
		}
			else
		{
	...
	case (_vehicle isKindOf "Ship"):
	{
		if(!local _vehicle) then
		{
			[_vehicle,(Fuel _vehicle) + 0.4] remoteExec ["life_fnc_setFuel", _vehicle, false];
		}
			else
		{
