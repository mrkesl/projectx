...
private["_tInv","_pInv","_veh_data"];
_isType = typeOf _veh;
if (!(_isType isEqualTo "CargoNet_01_box_F" OR _isType isEqualTo "B_supplyCrate_F" OR _isType isEqualTo "Box_IND_AmmoVeh_F")) then {
...
_getb = _veh getVariable["box",false];
if(_getb) then {
	_veh_data = [_veh] call life_fnc_vehicleWeightboxed;
	diag_log "mitbox";
	} else{
...
//Player Inventory Items
{
		...
		else
	{
		...
		{
			_pInv lbAdd format["$%1",[(player getvariable["cash",0])] call life_fnc_numberText];
