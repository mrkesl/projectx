...
private "_veh_data";
if(dialog) exitWith {};
_isType = typeOf _vehicle;
if(isNull _vehicle OR !(_vehicle isKindOf "Car" OR _vehicle isKindOf "Air" OR _vehicle isKindOf "Ship" OR _vehicle isKindOf "House_F" OR _isType isEqualTo "CargoNet_01_box_F" OR _isType isEqualTo "B_supplyCrate_F" OR _isType isEqualTo "Box_IND_AmmoVeh_F")) exitWith {}; //Either a null or invalid vehicle type.
...
_get = _vehicle getVariable["box", false];
if(_get) then {
	_veh_data = [_vehicle] call life_fnc_vehicleWeightboxed;
	diag_log "mitbox";
	} else{
...
if(_veh_data select 0 == -1 && {!(_vehicle isKindOf "House_F")} && {!(_isType isEqualTo "CargoNet_01_box_F")}) exitWith {closeDialog 0; _vehicle setVariable["trunk_in_use",false,true]; hint localize "STR_MISC_NoStorageVeh";};
...
_vehicle spawn
{
	...
	if(_this isKindOf "House_F") then {
		[_this] remoteExec ["TON_fnc_updateHouseTrunk", 2, false];
	};
};