...
if((player getvariable["cash",0]) < 1000) exitWith {hint localize "STR_Serive_Chopper_NotEnough"};
player setvariable["cash", (player getvariable["cash",0]) - 1000, true];
...
{
	[(_search select 0),1] remoteExec ["life_fnc_setFuel", (_search select 0), false];
}
	else
{
