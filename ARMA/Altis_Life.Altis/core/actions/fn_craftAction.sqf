...
diag_log format ["CRAFTING >>> _itemFilter = %1",_itemFilter];
_matsNeed = 0;
_weight = 0;
_curweap = "";
_weapName = "";
...
if(_itemFilter == "weapon" && primaryWeapon player != "") exitWith {
		_curweap = primaryWeapon player;
		_weapName = getText(configFile >> "CfgWeapons" >> (_curweap) >> "displayName");
        hint format["Du trägst bereits eine %1", _weapName];
};

if(_itemFilter == "launcher" && secondaryWeapon player != "") exitWith {
        _curweap = secondaryWeapon player;
		_weapName = getText(configFile >> "CfgWeapons" >> (_curweap) >> "displayName");
        hint format["Du trägst bereits eine %1", _weapName];
};

if(_itemFilter == "handgun" && handgunWeapon player != "") exitWith {
        _curweap = handgunWeapon player;
		_weapName = getText(configFile >> "CfgWeapons" >> (_curweap) >> "displayName");
        hint format["Du trägst bereits eine %1", _weapName];
};
...
if(_itemFilter == "backpack") then{
        if(backpack player == "") then{
                ...
                [_newItem] spawn life_fnc_texturesync;
        }else{
                hint localize "STR_CRAFT_AR_Backpack";
