/*
	File: fn_requestMedic.sqf
	Author: Bryan "Tonic" Boardwine
	
	Description:
	N/A
*/
private["_medicsOnline"];
_medicsOnline = {_x != player && {side _x == independent} && {alive _x}} count playableUnits > 0; //Check if medics (indep) are in the room.

if(_medicsOnline) then {
	//There is medics let's send them the request.
	[life_corpse,profileName] remoteExec ["life_fnc_medicRequest", independent, false];
	titleText["Notarzt wurde gerufen, nun warte auch auf diesen.","PLAIN"];
} else {
	//No medics were online, send it to the police.
	//[life_corpse,profileName] remoteExec ["life_fnc_copMedicRequest", west, false];
	((findDisplay 7300) displayCtrl 7303) ctrlEnable false;
	titleText["Es ist kein Notarzt im Dienst.","PLAIN"];
};
if(!_medicsOnline) exitWith {};
Life_request_timer = true;
//Create a thread to monitor duration since last request (prevent spammage).
[] spawn 
{
	((findDisplay 7300) displayCtrl 7303) ctrlEnable false;
	((findDisplay 7300) displayCtrl 7302) ctrlEnable false;
	sleep (life_respawn_timer * 450);
	((findDisplay 7300) displayCtrl 7303) ctrlEnable true;
	((findDisplay 7300) displayCtrl 7302) ctrlEnable true;
};