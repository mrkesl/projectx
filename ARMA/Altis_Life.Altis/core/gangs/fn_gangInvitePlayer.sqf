...
if(!isNil {(group _unit) getVariable "gang_name"}) exitWith {hint "Der Spieler ist bereits in einer Gang!"}; //Added
...
if(_action) then {
	[profileName,grpPlayer] remoteExec ["life_fnc_gangInvite", _unit, false];
