...
if!((getPlayerUID player) in (life_gangData select 5)) exitWith {player setVariable ["gang", "", true];};
...
if(!isNil "_group") then {
	...
	if((life_gangData select 1) == (getPlayerUID player)) then {
		...
		[player,_group] remoteExec ["TON_fnc_clientGangLeader", (units _group), false];
	};
} else {
	_group = group player;
