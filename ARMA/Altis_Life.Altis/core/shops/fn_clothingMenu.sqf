...
if((_this select 3) == "reb" && !(inarray("civ_rebel"))) exitWith {hint localize "STR_Shop_NotaReb"; closeDialog 0;};
if((_this select 3) in ["cop"] && playerSide != west) exitWith {hint localize "STR_Shop_NotaCop"; closeDialog 0;};
if((_this select 3) in ["dive"] && !(inarray("civ_dive"))) exitWith { hint localize "STR_Shop_NotaDive"; closeDialog 0;};
if((_this select 3) in ["medcloth"] && playerSide !=independent) exitWith {hint "Du bist kein Notzarzt"; closeDialog 0;};
if((_this select 3) in ["cl_ad"] && !life_onDuty) exitWith {hint "Du bist nicht im Dienst"; closeDialog 0;};
...
if(isNil "life_clothesPurchased") exitWith
{
	...
	if(life_oldClothes != "") then {player addUniform life_oldClothes;[life_oldClothes] spawn life_fnc_texturesync;} else {removeUniform player};
	...
	if(backpack player != "") then
	{
			...
			else
		{
			...
			[life_oldBackpack] spawn life_fnc_texturesync;
...
[uniform player] spawn life_fnc_texturesync;
};
...
{
	if(life_oldClothes != uniform player) then {player addUniform life_oldClothes;[life_oldClothes] spawn life_fnc_texturesync;};
};
//Check hat
