...
switch (true) do
{
	...
	case (_item == "redgull"):
	{
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			player setvariable["thirst", 100, true];
			player setFatigue 0;
			life_redgull_ammount = life_redgull_ammount +1;
			[] spawn
			{
				life_redgull_effect = time;
				titleText[localize "STR_ISTR_RedGullEffect","PLAIN"];
				player enableFatigue false;
				waitUntil {!alive player OR ((time - life_redgull_effect) > (life_redgull_ammount * 3 * 60))};
	...
	case (_item == "absperrung"):{["RoadBarrier_F",_item] spawn life_fnc_placeItem;};
	case (_item == "huetchen"):{["RoadCone_L_F",_item] spawn life_fnc_placeItem;};
	case (_item == "strahler"):{["Land_PortableLight_double_F",_item] spawn life_fnc_placeItem;};
	case (_item == "tapesign"):{["TapeSign_F",_item] spawn life_fnc_placeItem;};
	case (_item == "pole"):{["Pole_F",_item] spawn life_fnc_placeItem;};
	default{hint localize "STR_ISTR_NotUsable";};
	case (_item == "joint")://Grass
	{
		if(([false,_item,1] call life_fnc_handleInv)) then{
			[] spawn life_fnc_weed;};
	};
	case (_item == "cocainep")://Kokain
	{
		if(([false,_item,1] call life_fnc_handleInv)) then{
			[] spawn life_fnc_koka;};
	};
	case (_item == "heroinp")://Heroin
	{
		if(([false,_item,1] call life_fnc_handleInv)) then{
			[] spawn life_fnc_hero;};
	};
	...
	{
		if(([false,_item,1] call life_fnc_handleInv)) then{
			life_battery = 100;hint "Dein Akku ist jetzt wieder voll.";};
	};
	case (_item == "fuelPipe"):
    {
        [] spawn life_fnc_fuelPipe;
    };
	case (_item == "speedbomb"):
		{
			[] spawn life_fnc_usespeedbomb;
		};
};
