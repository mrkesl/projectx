...
_retrievePrice = [_className,__GETC__(life_pimpgarage_prices)] call TON_fnc_index;
_retrievePrice = if(_retrievePrice == -1) then {1000} else {(__GETC__(life_pimpgarage_prices) select _retrievePrice) select 1;};


(getControl(2800,2803)) ctrlSetStructuredText parseText format[
	"Bußgeld:" + " <t color='#8cff9b'>$%1</t><br/>
